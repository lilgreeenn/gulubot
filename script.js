...
function fetchWeather() {
    const apiKey = 'b8ad82a205f5c3a131a654973df8ff8c'; // 替换为您的API密钥
    const url = `https://api.openweathermap.org/data/2.5/weather?q=New%20York&appid=${apiKey}&units=metric`;

    fetch(url)
        .then(response => response.json())
        .then(data => {
            const temp = data.main.temp; // 获取温度
            const weatherDescription = data.weather[0].description; // 获取天气描述
...
function fetchRandomMovie() {
    const apiKey = '6df422f2c7cf4b0937257e719cde3af4'; // 替换为您的API密钥
    ...
    fetch(url)
        ...
        .then(data => {
            ...
            // Add click event listener to the movie poster
            document.getElementById('movie-poster').addEventListener('click', function() {
                fetchRandomMovie(); // Fetch and display a new random movie
            });
        })
        .catch(error => {
...
// Reuse the dragElement function provided earlier
function dragElement(element) {
    var pos1 = 0, pos2 = 0, pos3 = 0, pos4 = 0;
    element.onmousedown = dragMouseDown;
...
function sendMessage() {
    ...
    if (userText !== "") {
        ...
        if (userText.includes("hey")) {
            ...
            image.src = 'puppy.png';  // 确保这个路径是正确的
            image.alt = 'Cute puppy';
            image.style.display = 'block';
            image.style.margin = '10px auto';
            image.style.width = '40%'; // 设置图片宽度为容器的80%
            chatBox.appendChild(image);  // 直接将图片添加到聊天框中
        }

        if (userText.includes("dance")) {
            const video = document.createElement('video');  // 创建一个video元素
            video.src = 'result_ad.MP4';  // 设置视频文件的路径，确保路径正确
            video.alt = 'Dance video';  // 提供替代文本
            video.style.display = 'block';  // 显示样式为块级元素
            video.style.margin = '10px auto';  // 外边距，使其在容器中居中
            video.style.width = '40%';  // 视频的宽度
            video.controls = true;  // 提供控件，如播放、暂停等
            video.autoplay = true;  // 视频加载后自动播放（根据浏览器策略，可能需要用户交互才能播放）
            video.loop = true;  // 视频循环播放
        
            chatBox.appendChild(video);  // 将视频元素添加到聊天框中
        }
